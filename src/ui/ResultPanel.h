...
struct PotholeResult {
    ...
    double avgDepth;       ///< 平均深度 (mm)
    int pointCount;        ///< 点云数量
    bool isValid;          ///< 结果是否有效

    PotholeResult() 
        : depth(0.0), volume(0.0), area(0.0), width(0.0), length(0.0)
        , maxDepth(0.0), avgDepth(0.0), pointCount(0), isValid(false) {}
};
...
private:
    ...
    QLineEdit* m_avgDepthEdit;                   ///< 平均深度显示
